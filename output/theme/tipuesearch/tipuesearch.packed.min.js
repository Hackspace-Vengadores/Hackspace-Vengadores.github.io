...
if(c==1){out+='<div id="tipue_search_results_count">'+tipuesearch_string_4;}else{var c_c=c.toString().replace(/\B(?=(\d{3})+(?!\d))/g,",");out+='<div id="tipue_search_results_count">'+
c_c+" "+
...
out+="</div>";if(set.showRelated&&standard){var ront="";f=0;for(var i=0;i<tipuesearch_related.Related.length;i++){if(d==tipuesearch_related.Related[i].search){if(!f){out+='<div class="tipue_search_related">'+
tipuesearch_string_10+": ";}
...
ront+='<a class="tipue_search_related_btn" id="'+
r_d+'">'+
tipuesearch_related.Related[i].related+"</a>, ";f++;}}
if(f){ront=ront.slice(0,-2);ront+=".</div>";out+=ront;}}
if(show_replace){out+='<div id="tipue_search_replace">'+
tipuesearch_string_2+" "+
d+". "+
tipuesearch_string_3+' <a id="tipue_search_replaced">'+
d_r+"</a></div>";}
...
for(var i=0;i<found.length;i++){if(l_o>=start&&l_o<set.show+start){out+='<div class="tipue_search_result">';out+='<div class="tipue_search_content_title"><a href="'+
found[i].url+'"'+
tipue_search_w+">"+
found[i].title+"</a></div>";if(set.debug){out+='<div class="tipue_search_content_debug">Score: '+
found[i].score+"</div>";}
if(set.showURL){var s_u=found[i].url.toLowerCase();if(s_u.indexOf("http://")==0){s_u=s_u.slice(7);}
out+='<div class="tipue_search_content_url"><a href="'+
found[i].url+'"'+
tipue_search_w+">"+
s_u+"</a></div>";}
if(found[i].img){if(set.imageZoom){out+='<div class="tipue_search_image"><img class="tipue_search_img tipue_search_image_zoom" src="'+
found[i].img+'" alt="'+
found[i].title+'" data-url="'+
found[i].url+'"></div>';}else{out+='<div class="tipue_search_image"><a href="'+
found[i].url+'"'+
tipue_search_w+'><img class="tipue_search_img" src="'+
found[i].img+'" alt="'+
found[i].title+'"></a></div>';}}
...
if(found[i].note){out+='<div class="tipue_search_note">'+
found[i].note+"</div>";}
out+="</div>";}
l_o++;}
if(c>set.show){var pages=Math.ceil(c/set.show);var page=start/set.show;if(set.footerPages<3){set.footerPages=3;}
out+='<div id="tipue_search_foot"><ul id="tipue_search_foot_boxes">';if(start>0){out+='<li role="navigation"><a class="tipue_search_foot_box" accesskey="b" id="'+
(start-set.show)+"_"+
replace+'">'+
tipuesearch_string_6+"</a></li>";}
if(page<=2){var p_b=pages;if(pages>set.footerPages){p_b=set.footerPages;}
for(var f=0;f<p_b;f++){if(f==page){out+='<li class="current" role="navigation">'+
(f+1)+"</li>";}else{out+='<li role="navigation"><a class="tipue_search_foot_box" id="'+
f*set.show+"_"+
replace+'">'+
(f+1)+"</a></li>";}}}else{var p_b=page+set.footerPages-1;if(p_b>pages){p_b=pages;}
for(var f=page-1;f<p_b;f++){if(f==page){out+='<li class="current" role="navigation">'+
(f+1)+"</li>";}else{out+='<li role="navigation"><a class="tipue_search_foot_box" id="'+
f*set.show+"_"+
replace+'">'+
(f+1)+"</a></li>";}}}
if(page+1!=pages){out+='<li role="navigation"><a class="tipue_search_foot_box" accesskey="m" id="'+
(start+set.show)+"_"+
replace+'">'+
tipuesearch_string_7+"</a></li>";}
out+="</ul></div>";}}else{out+='<div id="tipue_search_error">'+tipuesearch_string_8+"</div>";}}else{if(show_stop){out+='<div id="tipue_search_error">'+
tipuesearch_string_8+" "+
tipuesearch_string_9+"</div>";}else{if(set.minimumLength==1){out+='<div id="tipue_search_error">'+
tipuesearch_string_11+"</div>";}else{out+='<div id="tipue_search_error">'+
tipuesearch_string_12+" "+
set.minimumLength+" "+
tipuesearch_string_13+"</div>";}}}
$("#tipue_search_content").hide().html(out).slideDown(200);$("#tipue_search_replaced").click(function(){getTipueSearch(0,false);});$(".tipue_search_related_btn").click(function(){$("#tipue_search_input").val($(this).attr("id"));getTipueSearch(0,true);});$(".tipue_search_image_zoom").click(function(){$("#tipue_search_image_modal").fadeIn(300);$("#tipue_search_zoom_img").attr("src",this.src);var z_u=$(this).attr("data-url");$("#tipue_search_zoom_url").attr("href",z_u);var z_o=this.alt+'<div class="tipue_search_zoom_options"><a href="'+
this.src+'" target="_blank">'+
tipuesearch_string_15+'</a>&nbsp; <a href="'+
z_u+'">'+
tipuesearch_string_16+"</a></div>";$("#tipue_search_zoom_text").html(z_o);});$(".tipue_search_image_close").click(function(){$("#tipue_search_image_modal").fadeOut(300);});$(".tipue_search_foot_box").click(function(){var id_v=$(this).attr("id");var id_a=id_v.split("_");getTipueSearch(parseInt(id_a[0]),id_a[1]);});}});};})(jQuery);